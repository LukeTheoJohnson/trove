"""adsbfi - live aircraft over a region via adsb.fi, a keyless community ADS-B network.

adsb.fi is a community-run, ad-free ADS-B aggregator (volunteer feeders) that publishes live aircraft
positions for public reuse at `opendata.adsb.fi/api/v2/lat/<lat>/lon/<lon>/dist/<nm>`: every aircraft
its network sees within <nm> nautical miles of a point - hex (transponder id), callsign, registration,
type, barometric + geometric altitude (feet), position, ground speed (kt), vertical rate (ft/min) and
squawk. The opendata host serves no robots.txt (404 = unfenced, the GBFS/S3 class) and exists to be
consumed = sanctioned -> trove. The second keyless-community twin of `opensky`/`adsblol`: same
"everything in the sky over a box right now" shape, a different feeder network + coverage.

The timeline value is un-rebuildable for a casual caller: an aircraft's altitude/position at a moment
is ephemeral and adsb.fi keeps no free per-aircraft history. `price_cents` = **barometric altitude in
feet** (so the core's `drops` = an aircraft *losing altitude* = descending on approach); `qty` = ground
speed (kt). A "deal" ("descending") = airborne, below 10,000 ft and sinking (baro_rate < 0) - a plane on
final approach over the region. money() renders altitude-feet as dollars in the two core-hardcoded spots.
...
`search <term>` filters by callsign/registration/type; `fetch` rescans the region for one hex.
"""
...
class _Client:
    ...
    def aircraft(self):
        if self._ac is None:
            ...
            r.raise_for_status()
            self._ac = (r.json() or {}).get("aircraft") or []
        return self._ac


class AdsbFiSource(Source):
    name = "adsbfi"
    id_label = "HEX"
    cc_default = "lon"
    ...
    def doctor(self, cl):
        ac = cl.aircraft()
        return bool(ac), f"({len(ac)} aircraft over '{cl.region}'; keyless adsb.fi community ADS-B)"
