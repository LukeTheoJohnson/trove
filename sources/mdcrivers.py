...
server (hydro.marlborough.govt.nz/data.hts) - the same open-data backend LAWA aggregates and the same
tech as `gwrivers` (Greater Wellington), but a different region (the top of the South Island: the
Wairau, Pelorus, Awatere and their tributaries). No key, no robots.txt (404 = nothing fenced),
official council open data = sanctioned -> trove.

The data is ephemeral *state*: a river's flow and stage (level) at telemetry cadence, changing with
rain and never archived in a convenient unified per-gauge series. As with gwrivers the flood-relevant
event is a **rise**, but trove's core only flags price *drops*, so this source computes the 24h trend
at fetch and stores `rising` as a flag: `is_deal` ("rising") = the latest value is >= 1.5x its value
24h ago (a flood-onset signal), while the core's `drops` = a river *receding*.
...
`fetch` reads Flow (m3/s) where available, else Stage (mm); `price_cents` = latest value * 100
(centi-cumecs / mm*100), with the measurement+unit in flags. `search <term>` fetches live flow for
gauges whose name matches; `--cc` is unused. Marlborough spaces its query with %20 (Hilltop does not
decode '+') - the same gotcha handled in gwrivers.
"""
...
class MDCRiversSource(Source):
    ...
    def doctor(self, cl):
        sites = cl.sites()
        return bool(sites), f"({len(sites)} Marlborough hydrology gauge sites; keyless Hilltop SiteList)"
    ...
    def format_item(self, item, obs):
        ...
        lines.append(f"  source      : Marlborough District Council Hilltop  ({e.get('url', '')})")
        return lines
