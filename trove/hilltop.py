"""Shared driver base for the keyless Hilltop hydrology class (ROADMAP §2).

Regional councils across NZ publish real-time river telemetry from the same Hilltop XML backend
(the open-data layer LAWA aggregates): `Request=SiteList` lists the gauge sites (join key = site
name); `Request=GetData&Site=<name>&Measurement=Flow&TimeInterval=PT24H/Now` returns the recent
series as `<E><T/><I1/></E>` points. Spaces must be %20-encoded (Hilltop does not decode '+').

The data is ephemeral *state*: flow/stage at telemetry cadence, changing with rain and never
archived in a convenient unified per-gauge series. The flood-relevant event is a **rise**, but the
core only flags price *drops*, so the 24h trend is computed at fetch time and stored as a `rising`
flag: `is_deal` ("rising") = latest value >= 1.5x its value 24h ago (a flood-onset signal), while
the core's `drops` = a river *receding*. `price_cents` = latest value * 100 (centi-cumecs for Flow,
mm*100 for Stage; per-site consistent - read `flags.measurement`/`unit`).

A council instance (gwrivers, mdcrivers, horizonsrivers, ...) is a ~15-line subclass setting
`name`, `host`, and `council` - gate the new host's robots first, then it's a config row.
"""
...
from .db import Item, Obs
from .session import retry_session, UA
from .tracker import Source, safe
...
def _report(host, site, measurement, unit, pts):
...
class HilltopClient:
    ...
    def _xml(self, params):
        ...
        url = f"{self.host}?{urlencode(params, quote_via=quote)}"
    ...
    def report(self, site):
        ...
        for m in ("Flow", "Stage"):
            try:
                ...
                pts, unit = [], ""
            if pts:
                return _report(self.host, site, m, _unit(unit) or ("m3/s" if m == "Flow" else "mm"), pts)
        return None


class HilltopRiversSource(Source):
    """Base for one council's Hilltop river source; a subclass sets name/host/council."""
    host = ""                    # the council's Data.hts endpoint
    council = ""                 # display label ("Greater Wellington")
    id_label = "SITE"
    ...
    def client(self, args):
        return HilltopClient(self.host)

    def doctor(self, cl):
        sites = cl.sites()
        return bool(sites), f"({len(sites)} {self.council} hydrology gauge sites; keyless Hilltop SiteList)"
    ...
    def format_item(self, item, obs):
        ...
        lines.append(f"  source      : {self.council} Hilltop  ({e.get('url', '')})")
        return lines
