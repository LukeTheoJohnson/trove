...
| mbhydro | **not money** — `price_cents` = **customers affected × 100** (centi-customer, `NUM_CUST_NOPOWER`), so `drops` = an outage *shrinking* (customers restored in stages); `qty` = crew-status ordinal (Initial Assessment 1 → Site Assessed 2 → attending 3 → Partially Restored 4 → Restored 5); deal "major" = **unplanned** & ≥ 100 customers. Join key = `OUTAGE_ID` (single network); an OUTAGE_ID gone from the feed = restored (the series ends). Polygon geometry reprojected via `outSR=4326`, first ring vertex = the coord. Dates are UTC epoch-ms rendered `YYYY-MM-DD HH:MMZ`. A `11111111` sentinel/test feature is skipped. money() renders centi-customers as $ in the watchlist + poll DROP line only | `customers`, `custtxt`, `status`, `cause`, `subcause`, `type`, `planned`, `etr`, `etr_verified`, `start`, `updated` | `outage_id`, `lat`, `lon`, `url` |
